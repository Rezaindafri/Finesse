File: **`server/routes/quests.js`**

1. Klik **"Add file"** → **"Create new file"**
2. Nama file: **`server/routes/quests.js`**
3. Paste ini:```js
...
router.post('/', (req, res) => {
  ...
  try {
    const result = db.prepare(`INSERT INTO quests (user_id, title, description, reason, total, exp_reward) VALUES (?, ?, ?, ?, ?, ?)`).run(user_id, title, description || '', reason || '', total, exp_reward || 100)
...
router.patch('/:id/progress', (req, res) => {
  ...
  try {
    ...
    db.prepare('UPDATE quests SET progress = ?, status = ? WHERE id = ?').run(newProgress, newStatus, req.params.id)
...
router.post('/:id/claim', (req, res) => {
  try {
    ...
    db.prepare("UPDATE quests SET status = 'claimed' WHERE id = ?").run(req.params.id)
    db.prepare('UPDATE users SET exp = exp + ? WHERE id = ?').run(quest.exp_reward, quest.user_id)
